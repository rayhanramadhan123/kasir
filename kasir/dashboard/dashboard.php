...
try {
    ...
    // Customer Baru (placeholder, as no customer table)
    $customer_baru = 0;
...
if (isset($_GET['action'])) {
    ...
    if (in_array($_GET['action'], ['add', 'edit', 'delete'])) {
...
<body class="min-h-screen flex">
    ...
    <div class="flex-1 flex flex-col">
        ...
        <div id="modal-overlay" class="modal fixed inset-0 bg-gray-900 bg-opacity-70 overflow-y-auto h-full w-full hidden opacity-0 z-50">
            <div class="modal-content relative top-20 mx-auto p-8 w-full max-w-lg">
                ...
                <div class="items-center mt-8 flex justify-end gap-3">
                    <button id="modal-cancel" class="px-5 py-2.5 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400">Batal</button>
        ...
        <header class="bg-white shadow-sm p-6 sticky top-0 z-40">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
                <h1 class="text-2xl font-bold text-indigo-700 flex items-center gap-2">
                    <i class="fas fa-shoe-prints"></i> Dashboard
                </h1>
        ...
        <main class="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 items-center">
                            ...
                            <select id="brand" aria-label="Filter brand sepatu" class="rounded-lg border border-gray-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white shadow-sm">
                                <option selected value="all">Semua Brand</option>
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between mb-6 gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-4">
                        <div class="relative w-full sm:w-72">
                            <input id="stock-search" aria-label="Search inventory" class="w-full rounded-lg border border-gray-200 px-4 py-2.5 pl-10 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white shadow-sm" placeholder="Cari stok..." type="search"/>
            ...
            <section aria-labelledby="transaction-title" class="bg-white rounded-xl shadow-sm p-8 overflow-x-auto scrollbar-thin">
                <div class="flex flex-col sm:flex-row items-center justify-between mb-6 gap-4">
                    ...
                    <div class="relative w-full sm:w-72">
                        <input id="transaction-search" aria-label="Search transactions" class="w-full rounded-lg border border-gray-200 px-4 py-2.5 pl-10 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white shadow-sm" placeholder="Cari transaksi..." type="search"/>
                        <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"></i>
                    </div>
                </div>
        ...
        <footer class="bg-white border-t border-gray-200 py-4 text-center text-gray-500 text-sm">
            © 2025 Toko Sepatu | Oke Shoes. All rights reserved.
        </footer>
    </div>
    ...
    document.getElementById('add-item').addEventListener('click', () => {
        openModal('Tambah Barang', `
            <form id="add-item-form">
                <div class="mb-4">
                    ...
                    <input type="text" id="kode-barang" class="mt-1 block w-full rounded-lg border border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 py-2 px-3" required>
        ...
        `, () => {
            ...
            fetch('dashboard.php?action=add', {
                method: 'POST',
    ...
    document.addEventListener('click', (e) => {
        if (e.target.closest('.edit-btn')) {
            ...
            openModal('Edit Barang', `
                <form id="edit-item-form">
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Kode Barang</label>
                        <input type="text" id="kode-barang" value="${item.kode_barang}" class="mt-1 block w-full rounded-lg border border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 py-2 px-3" required>
                        <p id="kode-barang-error" class="error-message hidden"></p>
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Nama Barang</label>
            ...
            `, () => {
                ...
                fetch('dashboard.php?action=edit', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        id_barang: id,
        ...
        if (e.target.closest('.delete-btn')) {
            ...
            openModal('Hapus Barang', `<p>Apakah Anda yakin ingin menghapus barang dengan ID ${id}?</p>`, () => {
                fetch('dashboard.php?action=delete', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        id_barang: id,
    ...
    openModal('Export to PDF', '<p>Apakah Anda yakin ingin mengekspor data ke PDF?</p>', () => {
        ...
        doc.autoTable({
            startY: doc.lastAutoTable.finalY + 15,
            theme: 'striped',
            head: [['ID Nota', 'Nama Barang', 'Qty', 'Total Harga']],
